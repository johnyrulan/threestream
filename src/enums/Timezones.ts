export default class Timezone {
  static TimezoneList = [
    {
      "id": "Pacific/Midway",
      "displayName": "(UTC-11:00) Midway Island, American Samoa",
      "utcOffset": -11
    },
    {
      "id": "Pacific/Honolulu",
      "displayName": "(UTC-10:00) Hawaii",
      "utcOffset": -10
    },
    {
      "id": "Pacific/Marquesas",
      "displayName": "(UTC-10:00) Marquesas Islands",
      "utcOffset": -10
    },
    {
      "id": "America/Adak",
      "displayName": "(UTC-10:00) Aleutian Islands",
      "utcOffset": -10
    },
    {
      "id": "America/Anchorage",
      "displayName": "(UTC-09:00) Alaska",
      "utcOffset": -9
    },
    {
      "id": "America/Tijuana",
      "displayName": "(UTC-08:00) Baja California",
      "utcOffset": -8
    },
    {
      "id": "America/Los_Angeles",
      "displayName": "(UTC-08:00) Pacific Time (US & Canada)",
      "utcOffset": -8
    },
    {
      "id": "America/Chihuahua",
      "displayName": "(UTC-07:00) Chihuahua, La Paz, Mazatlan",
      "utcOffset": -7
    },
    {
      "id": "America/Denver",
      "displayName": "(UTC-07:00) Mountain Time (US & Canada)",
      "utcOffset": -7
    },
    {
      "id": "America/Phoenix",
      "displayName": "(UTC-07:00) Arizona",
      "utcOffset": -7
    },
    {
      "id": "America/Chicago",
      "displayName": "(UTC-06:00) Central Time (US & Canada)",
      "utcOffset": -6
    },
    {
      "id": "America/Regina",
      "displayName": "(UTC-06:00) Saskatchewan",
      "utcOffset": -6
    },
    {
      "id": "Pacific/Easter",
      "displayName": "(UTC-06:00) Easter Island",
      "utcOffset": -6
    },
    {
      "id": "America/Mexico_City",
      "displayName": "(UTC-06:00) Guadalajara, Mexico City, Monterrey",
      "utcOffset": -6
    },
    {
      "id": "America/Belize",
      "displayName": "(UTC-06:00) Central America",
      "utcOffset": -6
    },
    {
      "id": "America/Bogota",
      "displayName": "(UTC-05:00) Bogota, Lima, Quito",
      "utcOffset": -5
    },
    {
      "id": "America/Havana",
      "displayName": "(UTC-05:00) Havana",
      "utcOffset": -5
    },
    {
      "id": "America/Port-au-Prince",
      "displayName": "(UTC-05:00) Haiti",
      "utcOffset": -5
    },
    {
      "id": "America/Indiana/Indianapolis",
      "displayName": "(UTC-05:00) Indiana (East)",
      "utcOffset": -5
    },
    {
      "id": "America/New_York",
      "displayName": "(UTC-05:00) Eastern Time (US & Canada)",
      "utcOffset": -5
    },
    {
      "id": "America/Cancun",
      "displayName": "(UTC-05:00) Chetumal",
      "utcOffset": -5
    },
    {
      "id": "America/Jamaica",
      "displayName": "(UTC-05:00) Jamaica",
      "utcOffset": -5
    },
    {
      "id": "America/Manaus",
      "displayName": "(UTC-04:00) Barbados, Georgetown, La Paz, Manaus, San Juan",
      "utcOffset": -4
    },
    {
      "id": "America/Halifax",
      "displayName": "(UTC-04:00) Atlantic Time (Canada)",
      "utcOffset": -4
    },
    {
      "id": "America/Santiago",
      "displayName": "(UTC-04:00) Santiago",
      "utcOffset": -4
    },
    {
      "id": "America/Asuncion",
      "displayName": "(UTC-04:00) Asuncion",
      "utcOffset": -4
    },
    {
      "id": "America/Cuiaba",
      "displayName": "(UTC-04:00) Cuiaba",
      "utcOffset": -4
    },
    {
      "id": "America/Caracas",
      "displayName": "(UTC-04:00) Caracas",
      "utcOffset": -4
    },
    {
      "id": "America/Grand_Turk",
      "displayName": "(UTC-04:00) Turks and Caicos",
      "utcOffset": -4
    },
    {
      "id": "America/St_Johns",
      "displayName": "(UTC-03:30) Newfoundland",
      "utcOffset": -3.5
    },
    {
      "id": "America/Montevideo",
      "displayName": "(UTC-03:00) Montevideo",
      "utcOffset": -3
    },
    {
      "id": "America/Bahia",
      "displayName": "(UTC-03:00) Salvador",
      "utcOffset": -3
    },
    {
      "id": "America/Cayenne",
      "displayName": "(UTC-03:00) Cayenne",
      "utcOffset": -3
    },
    {
      "id": "America/Sao_Paulo",
      "displayName": "(UTC-03:00) Brasilia",
      "utcOffset": -3
    },
    {
      "id": "America/Fortaleza",
      "displayName": "(UTC-03:00) Araguaina, Belem, Fortaleza",
      "utcOffset": -3
    },
    {
      "id": "America/Argentina/Buenos_Aires",
      "displayName": "(UTC-03:00) Buenos Aires",
      "utcOffset": -3
    },
    {
      "id": "America/Godthab",
      "displayName": "(UTC-03:00) Greenland",
      "utcOffset": -3
    },
    {
      "id": "America/Miquelon",
      "displayName": "(UTC-03:00) Saint Pierre and Miquelon",
      "utcOffset": -3
    },
    {
      "id": "America/Noronha",
      "displayName": "(UTC-02:00) Fernando de Noronha",
      "utcOffset": -2
    },
    {
      "id": "America/Azores",
      "displayName": "(UTC-01:00) Azores",
      "utcOffset": -1
    },
    {
      "id": "Atlantic/Cape_Verde",
      "displayName": "(UTC-01:00) Cabo Verde Islands",
      "utcOffset": -1
    },
    {
      "id": "Africa/Abidjan",
      "displayName": "(UTC) Abidjan",
      "utcOffset": 0
    },
    {
      "id": "Africa/Monrovia",
      "displayName": "(UTC) Monrovia, Reykjavik",
      "utcOffset": 0
    },
    {
      "id": "Africa/Accra",
      "displayName": "(UTC) Accra",
      "utcOffset": 0
    },
    {
      "id": "Africa/Bissau",
      "displayName": "(UTC) Bissau",
      "utcOffset": 0
    },
    {
      "id": "Europe/London",
      "displayName": "(UTC) Dublin, Edinburgh, Lisbon, London",
      "utcOffset": 0
    },
    {
      "id": "Africa/Casablanca",
      "displayName": "(UTC) Casablanca",
      "utcOffset": 0
    },
    {
      "id": "Africa/Algiers",
      "displayName": "(UTC+01:00) West Central Africa",
      "utcOffset": 1
    },
    {
      "id": "Africa/Ceuta",
      "displayName": "(UTC+01:00) Ceuta",
      "utcOffset": 1
    },
    {
      "id": "Europe/Amsterdam",
      "displayName": "(UTC+01:00) Amsterdam, Berlin, Bern, Rome, Stockholm, Vienna",
      "utcOffset": 1
    },
    {
      "id": "Europe/Belgrade",
      "displayName": "(UTC+01:00) Belgrade, Bratislava, Budapest, Ljubljana, Prague",
      "utcOffset": 1
    },
    {
      "id": "Europe/Brussels",
      "displayName": "(UTC+01:00) Brussels, Copenhagen, Madrid, Paris",
      "utcOffset": 1
    },
    {
      "id": "Europe/Warsaw",
      "displayName": "(UTC+01:00) Sarajevo, Skopje, Warsaw, Zagreb",
      "utcOffset": 1
    },
    {
      "id": "Africa/Windhoek",
      "displayName": "(UTC+01:00) Windhoek",
      "utcOffset": 1
    },
    {
      "id": "Africa/Tripoli",
      "displayName": "(UTC+02:00) Tripoli",
      "utcOffset": 2
    },
    {
      "id": "Africa/Cairo",
      "displayName": "(UTC+02:00) Cairo",
      "utcOffset": 2
    },
    {
      "id": "Asia/Gaza",
      "displayName": "(UTC+02:00) Gaza, Hebron",
      "utcOffset": 2
    },
    {
      "id": "Europe/Helsinki",
      "displayName": "(UTC+02:00) Helsinki, Kyiv, Riga, Sofia, Tallinn, Vilnius",
      "utcOffset": 2
    },
    {
      "id": "Asia/Beirut",
      "displayName": "(UTC+02:00) Beirut",
      "utcOffset": 2
    },
    {
      "id": "Africa/Johannesburg",
      "displayName": "(UTC+02:00) Johannesburg, Pretoria",
      "utcOffset": 2
    },
    {
      "id": "Asia/Jerusalem",
      "displayName": "(UTC+02:00) Jerusalem",
      "utcOffset": 2
    },
    {
      "id": "Europe/Athens",
      "displayName": "(UTC+02:00) Athens, Bucharest",
      "utcOffset": 2
    },
    {
      "id": "Europe/Kaliningrad",
      "displayName": "(UTC+02:00) Kaliningrad",
      "utcOffset": 2
    },
    {
      "id": "Asia/Damascus",
      "displayName": "(UTC+02:00) Damascus",
      "utcOffset": 2
    },
    {
      "id": "Africa/Harare",
      "displayName": "(UTC+02:00) Harare",
      "utcOffset": 2
    },
    {
      "id": "Asia/Kuwait",
      "displayName": "(UTC+03:00) Kuwait, Riyadh",
      "utcOffset": 3
    },
    {
      "id": "Europe/Minsk",
      "displayName": "(UTC+03:00) Minsk",
      "utcOffset": 3
    },
    {
      "id": "Asia/Istanbul",
      "displayName": "(UTC+03:00) Istanbul",
      "utcOffset": 3
    },
    {
      "id": "Europe/Moscow",
      "displayName": "(UTC+03:00) Moscow, St. Petersburg, Volgograd",
      "utcOffset": 3
    },
    {
      "id": "Asia/Baghdad",
      "displayName": "(UTC+03:00) Baghdad",
      "utcOffset": 3
    },
    {
      "id": "Asia/Amman",
      "displayName": "(UTC+03:00) Amman",
      "utcOffset": 3
    },
    {
      "id": "Africa/Nairobi",
      "displayName": "(UTC+03:00) Nairobi",
      "utcOffset": 3
    },
    {
      "id": "Asia/Tehran",
      "displayName": "(UTC+03:30) Tehran",
      "utcOffset": 3.5
    },
    {
      "id": "Europe/Samara",
      "displayName": "(UTC+04:00) Izhevsk, Samara",
      "utcOffset": 4
    },
    {
      "id": "Europe/Astrakhan",
      "displayName": "(UTC+04:00) Astrakhan, Ulyanovsk",
      "utcOffset": 4
    },
    {
      "id": "Asia/Tbilisi",
      "displayName": "(UTC+04:00) Tbilisi",
      "utcOffset": 4
    },
    {
      "id": "Asia/Baku",
      "displayName": "(UTC+04:00) Baku",
      "utcOffset": 4
    },
    {
      "id": "Asia/Yerevan",
      "displayName": "(UTC+04:00) Yerevan",
      "utcOffset": 4
    },
    {
      "id": "Indian/Mauritius",
      "displayName": "(UTC+04:00) Port Louis",
      "utcOffset": 4
    },
    {
      "id": "Asia/Dubai",
      "displayName": "(UTC+04:00) Abu Dhabi, Muscat",
      "utcOffset": 4
    },
    {
      "id": "Asia/Kabul",
      "displayName": "(UTC+04:30) Kabul",
      "utcOffset": 4.5
    },
    {
      "id": "Asia/Yekaterinburg",
      "displayName": "(UTC+05:00) Ekaterinburg",
      "utcOffset": 5
    },
    {
      "id": "Asia/Tashkent",
      "displayName": "(UTC+05:00) Tashkent, Ashgabat",
      "utcOffset": 5
    },
    {
      "id": "Asia/Karachi",
      "displayName": "(UTC+05:00) Islamabad, Karachi",
      "utcOffset": 5
    },
    {
      "id": "Asia/Kolkata",
      "displayName": "(UTC+05:30) Chennai, Kolkata, Mumbai, New Delhi",
      "utcOffset": 5.5
    },
    {
      "id": "Asia/Colombo",
      "displayName": "(UTC+05:30) Sri Jayawardenepura",
      "utcOffset": 5.5
    },
    {
      "id": "Asia/Kathmandu",
      "displayName": "(UTC+05:45) Kathmandu",
      "utcOffset": 5.75
    },
    {
      "id": "Asia/Dhaka",
      "displayName": "(UTC+06:00) Dhaka",
      "utcOffset": 6
    },
    {
      "id": "Indian/Chagos",
      "displayName": "(UTC+06:00) Chagos",
      "utcOffset": 6
    },
    {
      "id": "Asia/Almaty",
      "displayName": "(UTC+06:00) Astana",
      "utcOffset": 6
    },
    {
      "id": "Asia/Rangoon",
      "displayName": "(UTC+06:30) Yangon (Rangoon)",
      "utcOffset": 6.5
    },
    {
      "id": "Asia/Krasnoyarsk",
      "displayName": "(UTC+07:00) Krasnoyarsk",
      "utcOffset": 7
    },
    {
      "id": "Asia/Bangkok",
      "displayName": "(UTC+07:00) Bangkok, Hanoi, Jakarta",
      "utcOffset": 7
    },
    {
      "id": "Asia/Barnaul",
      "displayName": "(UTC+07:00) Barnaul, Gorno-Altaysk",
      "utcOffset": 7
    },
    {
      "id": "Asia/Hovd",
      "displayName": "(UTC+07:00) Hovd",
      "utcOffset": 7
    },
    {
      "id": "Asia/Novosibirsk",
      "displayName": "(UTC+07:00) Novosibirsk",
      "utcOffset": 7
    },
    {
      "id": "Asia/Tomsk",
      "displayName": "(UTC+07:00) Tomsk",
      "utcOffset": 7
    },
    {
      "id": "Asia/Choibalsan",
      "displayName": "(UTC+08:00) Ulaanbaatar",
      "utcOffset": 8
    },
    {
      "id": "Asia/Chongqing",
      "displayName": "(UTC+08:00) Beijing, Chongqing, Hong Kong, Urumqi",
      "utcOffset": 8
    },
    {
      "id": "Asia/Irkutsk",
      "displayName": "(UTC+08:00) Irkutsk",
      "utcOffset": 8
    },
    {
      "id": "Australia/Perth",
      "displayName": "(UTC+08:00) Perth",
      "utcOffset": 8
    },
    {
      "id": "Asia/Taipei",
      "displayName": "(UTC+08:00) Taipei",
      "utcOffset": 8
    },
    {
      "id": "Asia/Kuala_Lumpur",
      "displayName": "(UTC+08:00) Kuala Lumpur, Singapore",
      "utcOffset": 8
    },
    {
      "id": "Asia/Pyongyang",
      "displayName": "(UTC+08:30) Pyongyang",
      "utcOffset": 8.5
    },
    {
      "id": "Australia/Eucla",
      "displayName": "(UTC+08:45) Eucla",
      "utcOffset": 8.75
    },
    {
      "id": "Asia/Seoul",
      "displayName": "(UTC+09:00) Seoul",
      "utcOffset": 9
    },
    {
      "id": "Asia/Chita",
      "displayName": "(UTC+09:00) Chita",
      "utcOffset": 9
    },
    {
      "id": "Asia/Tokyo",
      "displayName": "(UTC+09:00) Osaka, Sapporo, Tokyo",
      "utcOffset": 9
    },
    {
      "id": "Asia/Yakutsk",
      "displayName": "(UTC+09:00) Yakutsk",
      "utcOffset": 9
    },
    {
      "id": "Australia/Darwin",
      "displayName": "(UTC+09:30) Darwin",
      "utcOffset": 9.5
    },
    {
      "id": "Australia/Adelaide",
      "displayName": "(UTC+09:30) Adelaide",
      "utcOffset": 9.5
    },
    {
      "id": "Australia/Sydney",
      "displayName": "(UTC+10:00) Canberra, Melbourne, Sydney",
      "utcOffset": 10
    },
    {
      "id": "Pacific/Guam",
      "displayName": "(UTC+10:00) Guam, Port Moresby",
      "utcOffset": 10
    },
    {
      "id": "Australia/Hobart",
      "displayName": "(UTC+10:00) Hobart",
      "utcOffset": 10
    },
    {
      "id": "Australia/Brisbane",
      "displayName": "(UTC+10:00) Brisbane",
      "utcOffset": 10
    },
    {
      "id": "Asia/Vladivostok",
      "displayName": "(UTC+10:00) Vladivostok",
      "utcOffset": 10
    },
    {
      "id": "Australia/Lord_Howe",
      "displayName": "(UTC+10:30) Lord Howe Island",
      "utcOffset": 10.5
    },
    {
      "id": "Pacific/Bougainville",
      "displayName": "(UTC+11:00) Bougainville Island",
      "utcOffset": 11
    },
    {
      "id": "Pacific/Norfolk",
      "displayName": "(UTC+11:00) Norfolk Island",
      "utcOffset": 11
    },
    {
      "id": "Asia/Sakhalin",
      "displayName": "(UTC+11:00) Sakhalin",
      "utcOffset": 11
    },
    {
      "id": "Asia/Magadan",
      "displayName": "(UTC+11:00) Magadan",
      "utcOffset": 11
    },
    {
      "id": "Asia/Srednekolymsk",
      "displayName": "(UTC+11:00) Chokirdakh",
      "utcOffset": 11
    },
    {
      "id": "Pacific/Guadalcanal",
      "displayName": "(UTC+11:00) Solomon Islands, New Caledonia",
      "utcOffset": 11
    },
    {
      "id": "Pacific/Auckland",
      "displayName": "(UTC+12:00) Auckland, Wellington",
      "utcOffset": 12
    },
    {
      "id": "Pacific/Fiji",
      "displayName": "(UTC+12:00) Fiji Islands",
      "utcOffset": 12
    },
    {
      "id": "Asia/Anadyr",
      "displayName": "(UTC+12:00) Anadyr - Petropavlovsk-Kamchatskiy",
      "utcOffset": 12
    },
    {
      "id": "Pacific/Tarawa",
      "displayName": "(UTC+12:00) Tarawa",
      "utcOffset": 12
    },
    {
      "id": "Pacific/Wake",
      "displayName": "(UTC+12:00) Wake",
      "utcOffset": 12
    },
    {
      "id": "Pacific/Wallis",
      "displayName": "(UTC+12:00) Wallis",
      "utcOffset": 12
    },
    {
      "id": "Pacific/Chatham",
      "displayName": "(UTC+12:45) Chatham Islands",
      "utcOffset": 12.75
    },
    {
      "id": "Pacific/Tongatapu",
      "displayName": "(UTC+13:00) Nuku'alofa",
      "utcOffset": 13
    },
    {
      "id": "Pacific/Apia",
      "displayName": "(UTC+13:00) Samoa",
      "utcOffset": 13
    },
    {
      "id": "Pacific/Kiritimati",
      "displayName": "(UTC+14:00) Kiritimati Island",
      "utcOffset": 14
    }
  ];

  static findTimezone(id: string) {
    return Timezone.TimezoneList.filter(t => t.id === id)[0];
  }
}